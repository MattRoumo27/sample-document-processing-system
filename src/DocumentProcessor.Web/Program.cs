...
try
{
    ...
    try
    {
        ...
        if (!string.IsNullOrWhiteSpace(secretJson))
        {
            ...
            var dbname = "postgres";
            connectionString = $"Host={host};Port={port};Database=postgres;Username={username};Password={password};SSL Mode=Require;Trust Server Certificate=true";
            dbInfo.DatabaseType = "PostgreSQL"; dbInfo.SecretName = "arn:aws:secretsmanager:us-east-1:779846805432:secret:atx-db-modernization-atx-db-modernization-1-target-OCfJnG"; dbInfo.HostAddress = $"{host}:{port}";
        }
...
catch (Exception ex)
{
    ...
    connectionString = builder.Configuration.GetConnectionString("DefaultConnection") ?? "Host=localhost;Database=postgres;Username=admin;Password=";
